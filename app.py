    ...
    if st.button("ACESSAR"):
        ...
        if res.data and res.data[0]["senha"] == p:
            st.session_state.update({
                "autenticado": True, 
                "user_slug": u, 
                "user_nome": res.data[0]["nome"], 
                "user_setores": res.data[0]["setores"]
            })
            st.rerun()
...
# --- 2. SELEÇÃO DE SETORES ---
    ...
    for i, s in enumerate(setores_visiveis):
        with cols[i % 3]:
            tipo = "primary" if "ADMIN" in s else "secondary"
    ...
    if setor_atual == "🔐 PAINEL ADMIN":
        if not st.session_state["admin_liberado"]:
            ...
            with st.form("f_senha"):
                u_sel = st.selectbox("Selecione o Usuário:", df_u['login'].tolist())
    ...
    # --- SETORES OPERACIONAIS ---
        ...
        with tab_ia:
            ...
            if prompt := st.chat_input("Fale com o sistema..."):
                ...
                with st.chat_message("assistant"):
                    ...
                    contexto_web = busca_web(prompt) if any(x in prompt.lower() for x in ["hoje", "dólar", "agora", "cotação"]) else ""
                    ctx = f"Você é a IA da JNL. REGRAS: {regras}. WEB: {contexto_web}"
        ...
        with tab_chat:
            ...
            for m in res_c.data:
                ...
                conteudo = "<i>🚫 Mensagem apagada</i>" if m['apagada'] == 1 else f"{m['mensagem']} {'*(Editada)*' if m['editada'] == 1 else ''}"
                st.markdown(f"<div style='text-align: {align};'><div style='display: inline-block; background: {bg}; padding: 10px; border-radius: 10px; color: black; margin: 5px; min-width: 150px;'><b>{m['usuario'].upper()}</b><br>{conteudo}<br><small>{m['data_hora']}</small></div></div>", unsafe_allow_html=True)
                ...
                if is_me and m['apagada'] == 0:
                    ...
                    if (datetime.now(ts.tzinfo) - ts).total_seconds() / 60 <= 20:
                        if st.button("🗑️ Apagar", key=f"a_{m['id']}"):
                            supabase.table("chat_setor").update({"apagada": 1}).eq("id", m['id']).execute(); st.rerun()

            m_txt = st.text_input("Escreva sua mensagem...")
            if st.button("Enviar"):
                if m_txt:
                    agora = get_now_br()
        ...
        with tab_age:
            with st.form("f_age", clear_on_submit=True):
                ...
                dest = st.text_input("E-mail Alerta", placeholder="email@jnl.com.br")
                if st.form_submit_button("Agendar"):
                    supabase.table("calendario").insert({
                        "usuario": user, 
                        "titulo": t, 
                        "data_hora": f"{d} {h}:{mi}:00", 
                        "destinatarios": dest
                    }).execute(); st.rerun()
