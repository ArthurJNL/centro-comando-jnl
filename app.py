...
def init_db():
    ...
    # Atualização de tabelas (segurança caso já existam)
...
# --- 3. DASHBOARD PRINCIPAL ---
    ...
    # --- BARRA LATERAL (COM PLANILHAS DE VOLTA) ---
    with st.sidebar:
        st.header(f"📍 {setor_atual}")
        if st.button("⬅️ TROCAR DE SETOR"): st.session_state["setor"] = None; st.rerun()
        st.write("---")
    ...
    if is_admin: abas_nomes.append("🔐 ADMIN")
    ...
    with abas[0]:
        ...
        if prompt := st.chat_input("Como posso ajudar a JNL hoje?"):
            ...
            with st.chat_message("assistant"):
                try:
                    # IA busca na internet silenciosamente se necessário
    ...
    if is_admin:
        with abas[4]:
            st.subheader("Painel de Controle e Senhas")
            st.warning("Área restrita à diretoria (Arthur, Felipe e Pedro).")
            conn = sqlite3.connect('jnl_master.db')
            ...
            with st.form("form_senha"):
                ...
                if st.form_submit_button("Trocar Senha"):
                    ...
                    conn.commit()
                    st.success(f"Senha de {usr_sel.upper()} atualizada com sucesso!")
            conn.close()